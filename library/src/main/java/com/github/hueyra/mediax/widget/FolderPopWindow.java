...
import com.github.hueyra.mediax.adapter.PictureAlbumDirectoryAdapter;
    ...
    public FolderPopWindow(Context context) {
        ...
        if (PictureSelectionConfig.uiStyle != null) {
            ...
            if (config.isWeChatStyle) {
                this.drawableUp = ContextCompat.getDrawable(context, R.mipmap.picture_icon_wechat_up);
                this.drawableDown = ContextCompat.getDrawable(context, R.mipmap.picture_icon_wechat_down);
            } else {
                if (config.upResId != 0) {
                    this.drawableUp = ContextCompat.getDrawable(context, config.upResId);
                } else {
                    // 兼容老的Theme方式
                    this.drawableUp = AttrsUtils.getTypeValueDrawable(context, R.attr.picture_arrow_up_icon, R.mipmap.picture_icon_arrow_up);
                }
                if (config.downResId != 0) {
                    ...
                    this.drawableDown = AttrsUtils.getTypeValueDrawable(context, R.attr.picture_arrow_down_icon, R.mipmap.picture_icon_arrow_down);
                }
            }
        }
