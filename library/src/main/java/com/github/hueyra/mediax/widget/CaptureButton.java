...
public class CaptureButton extends View {
    ...
    public final int progress_color = 0xEE16AE16;            //进度条颜色
    public final int outside_color = 0xEEDCDCDC;             //外圆背景色
    public final int inside_color = 0xFFFFFFFF;              //内圆背景色
    ...
    public CaptureButton(Context context, int size) {
        ...
        button_state = Config.BUTTON_STATE_BOTH;  //初始化按钮为可录制可拍照
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                //判断按钮状态是否为可录制状态
                if ((button_state == Config.BUTTON_STATE_ONLY_RECORDER || button_state == Config.BUTTON_STATE_BOTH))
                    postDelayed(longPressRunnable, 500);    //同时延长500启动长按后处理的逻辑Runnable
                break;
            case MotionEvent.ACTION_MOVE:
                if (captureListener != null
                        && state == STATE_RECORDING
                        && (button_state == Config.BUTTON_STATE_ONLY_RECORDER || button_state == Config.BUTTON_STATE_BOTH)) {
    ...
    private void handlerPressByState() {
        ...
        switch (state) {
            //当前是点击按下
            case STATE_PRESS:
                if (captureListener != null && (button_state == Config.BUTTON_STATE_ONLY_CAPTURE || button_state ==
                        Config.BUTTON_STATE_BOTH)) {
                    startCaptureAnimation(button_inside_radius);
                } else {
                    state = STATE_IDLE;
                }
                break;
            // ---CodeReview---当内外圆动画未结束时已经是长按状态 但还没有置为STATE_RECORDING时 应该也要结束录制  此处是一个bug
    ...
    private class LongPressRunnable implements Runnable {
        ...
        public void run() {
            ...
            //没有录制权限
            if (Utils.getRecordState() != Config.CHECK_PERMISSION_STATE_SUCCESS) {
                state = STATE_IDLE;
                if (captureListener != null) {
