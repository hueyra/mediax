...
public class CropViewLayout extends RelativeLayout {
    ...
    private static final String TAG = "CropViewLayout";

    //裁剪原图
    private ImageView mImageView;
    //裁剪框
    private CropView mClipView;
    ...
    public CropViewLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CropViewLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void init(Context context, AttributeSet attrs) {
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.CropViewLayout);

        //获取剪切框距离左右的边距, 默认为50dp
        mHorizontalPadding = array.getDimensionPixelSize(R.styleable.CropViewLayout_horizontalPadding,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50, getResources().getDisplayMetrics()));
        //获取裁剪框边框宽度，默认1dp
        int clipBorderWidth = array.getDimensionPixelSize(R.styleable.CropViewLayout_cropBorderWidth,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, getResources().getDisplayMetrics()));
        //裁剪框类型(圆或者矩形)
        int clipType = array.getInt(R.styleable.CropViewLayout_cropType, 2);

        //回收
        array.recycle();


        mClipView = new CropView(context);
    ...
    public void initSrcPic(Uri uri) {
        ...
        try {
            ...
            if (uri.getScheme() != null && ContentResolver.SCHEME_CONTENT.equalsIgnoreCase(uri.getScheme())) {
                //content
            ...
            int rotation = getExifOrientation(uri); //查询旋转角度
            if (rotation > 0) {
                Matrix m = new Matrix();
     ...
     * 查询图片旋转角度
     */
    public int getExifOrientation(Uri uri) {// YOUR MEDIA PATH AS STRING
        int degree = 0;
        if (Build.VERSION.SDK_INT > 100) {
            String d = UriUtils.getImgOrientationFromUri(getContext(), uri);
            if (!TextUtils.isEmpty(d)) {
                degree = Integer.parseInt(d);
            }
        } else {
            String path = UriUtils.getRealFilePathFromUri(getContext(), uri);
    ...
    public Bitmap crop() {
        ...
        try {
            ...
            zoomedCropBitmap = BitmapUtils.zoomBitmap(cropBitmap, mPreViewW, mPreViewW);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (cropBitmap != null) {
