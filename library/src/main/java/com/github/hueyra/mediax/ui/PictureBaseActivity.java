...
import com.github.hueyra.mediax.compress.Luban;
import com.github.hueyra.mediax.compress.OnCompressListener;
...
import com.github.hueyra.mediax.dialog.PictureLoadingDialog;
...
import com.github.hueyra.mediax.tools.AndroidQTransformUtils;
...
public abstract class PictureBaseActivity extends AppCompatActivity {
    ...
    private void newCreateEngine() {
        if (PictureSelectionConfig.imageEngine == null) {
            PictureSelectorEngine baseEngine = MediaXAppInitializer.getInstance().getPictureSelectorEngine();
    ...
    protected void onSaveInstanceState(@NotNull Bundle outState) {
        ...
        outState.putParcelable(Config.EXTRA_CONFIG, config);
    }
    ...
    protected void onResult(List<LocalMedia> images) {
        ...
        if (isAndroidQ && config.isAndroidQTransform) {
            ...
            dismissDialog();
            if (config.camera
                    && config.selectionMode == Config.MULTIPLE
    ...
    protected void exit() {
        finish();
        if (config.camera) {
            overridePendingTransition(0, R.anim.mx_anim_fade_out);
    ...
    protected void startOpenCamera() {
        ...
        if (cameraIntent.resolveActivity(getPackageManager()) != null) {
            ...
            int chooseMode = config.chooseMode == Config.TYPE_ALL ? Config.TYPE_IMAGE : config.chooseMode;
            ...
            if (config.isCameraAroundState) {
                cameraIntent.putExtra(Config.CAMERA_FACING, Config.CAMERA_BEFORE);
            }
            cameraIntent.putExtra(MediaStore.EXTRA_OUTPUT, imageUri);
            startActivityForResult(cameraIntent, Config.REQUEST_OPEN_CAMERA);
        }
    }
    ...
    protected void startOpenCameraVideo() {
        ...
        if (cameraIntent.resolveActivity(getPackageManager()) != null) {
            Uri videoUri;
            String cameraFileName = null;
            int chooseMode = config.chooseMode == Config.TYPE_ALL ? Config.TYPE_VIDEO : config.chooseMode;
            if (!TextUtils.isEmpty(config.cameraFileName)) {
                boolean isSuffixOfImage = PictureMimeType.isSuffixOfImage(config.cameraFileName);
            ...
            cameraIntent.putExtra(Config.EXTRA_QUICK_CAPTURE, config.isQuickCapture);
    ...
    public void startOpenCameraAudio() {
        if (PermissionChecker.checkSelfPermission(this, Manifest.permission.RECORD_AUDIO)) {
            ...
            PermissionChecker.requestPermissions(this,
                    new String[]{Manifest.permission.RECORD_AUDIO}, Config.APPLY_AUDIO_PERMISSIONS_CODE);
        }
    }
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == Config.APPLY_AUDIO_PERMISSIONS_CODE) {
