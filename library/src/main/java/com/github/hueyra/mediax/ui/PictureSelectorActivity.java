...
import com.github.hueyra.mediax.PictureSelector;
import com.github.hueyra.mediax.R;
import com.github.hueyra.mediax.adapter.PictureImageGridAdapter;
import com.github.hueyra.mediax.animators.AlphaInAnimationAdapter;
import com.github.hueyra.mediax.animators.AnimationType;
import com.github.hueyra.mediax.animators.SlideInBottomAnimationAdapter;
...
import com.github.hueyra.mediax.dialog.PhotoItemSelectedDialog;
import com.github.hueyra.mediax.dialog.PictureCustomDialog;
...
import com.github.hueyra.mediax.listener.OnItemClickListener;
import com.github.hueyra.mediax.listener.OnPhotoSelectChangedListener;
import com.github.hueyra.mediax.listener.OnQueryDataResultListener;
import com.github.hueyra.mediax.listener.OnRecyclerViewPreloadMoreListener;
import com.github.hueyra.mediax.manager.UCropManager;
import com.github.hueyra.mediax.model.CutInfo;
import com.github.hueyra.mediax.loader.LocalMediaLoader;
import com.github.hueyra.mediax.loader.LocalMediaPageLoader;
import com.github.hueyra.mediax.observable.ImagesObservable;
import com.github.hueyra.mediax.other.PictureMediaScannerConnection;
...
import com.github.hueyra.mediax.tools.BitmapUtils;
import com.github.hueyra.mediax.tools.DateUtils;
import com.github.hueyra.mediax.tools.DoubleUtils;
import com.github.hueyra.mediax.tools.JumpUtils;
import com.github.hueyra.mediax.tools.MediaUtils;
import com.github.hueyra.mediax.tools.PictureFileUtils;
...
import com.github.hueyra.mediax.widget.FolderPopWindow;
...
public class PictureSelectorActivity extends PictureBaseActivity implements View.OnClickListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            allFolderSize = savedInstanceState.getInt(Config.EXTRA_ALL_FOLDER_SIZE);
            oldCurrentListSize = savedInstanceState.getInt(Config.EXTRA_OLD_CURRENT_LIST_SIZE, 0);
    ...
    protected void initWidgets() {
        ...
        mBottomLayout.setVisibility(config.selectionMode == Config.SINGLE
        ...
        mRecyclerView.addItemDecoration(new GridSpacingItemDecoration(config.imageSpanCount <= 0 ? Config.DEFAULT_SPAN_COUNT : config.imageSpanCount,
                ScreenUtils.dip2px(this, 2), false));
        mRecyclerView.setLayoutManager(new GridLayoutManager(getContext(), config.imageSpanCount <= 0 ? Config.DEFAULT_SPAN_COUNT : config.imageSpanCount));
                        ...
                        (OnQueryDataResultListener<LocalMedia>) (result, currentPage, isHasMore) -> {
                            if (!isFinishing()) {
                                ...
                                if (isHasMore) {
                                    ...
                                    if (size < Config.MIN_PAGE_SIZE) {
                                        mRecyclerView.onScrolled(mRecyclerView.getScrollX(), mRecyclerView.getScrollY());
                                    }
                                } else {
                                    boolean isEmpty = mAdapter.isDataEmpty();
                                    if (isEmpty) {
                                        showDataNull(bucketId == -1 ? getString(R.string.picture_empty) : getString(R.string.picture_data_null), R.mipmap.picture_icon_no_data);
                                    }
                                }
                            }
                        });
    ...
    private void loadAllMediaData() {
        if (PermissionChecker
            ...
            PermissionChecker.requestPermissions(this, new String[]{
                    ...
                    Manifest.permission.WRITE_EXTERNAL_STORAGE}, Config.APPLY_STORAGE_PERMISSIONS_CODE);
        }
    }
    ...
    public void initPictureSelectorStyle() {
        if (PictureSelectionConfig.uiStyle != null) {
            ...
            Drawable leftDrawable = AttrsUtils.getTypeValueDrawable(getContext(), R.attr.picture_leftBack_icon, R.mipmap.picture_icon_close);
            ...
            if (config.downResId != 0) {
                ...
                Drawable downDrawable = AttrsUtils.getTypeValueDrawable(getContext(), R.attr.picture_arrow_down_icon, R.mipmap.picture_icon_arrow_down);
    ...
    protected void onSaveInstanceState(@NotNull Bundle outState) {
        ...
        if (mAdapter != null) {
            ...
            outState.putInt(Config.EXTRA_OLD_CURRENT_LIST_SIZE, mAdapter.getSize());
            // Save the number of Camera film and Camera folder files
            int size = folderWindow.getFolderData().size();
            if (size > 0) {
                outState.putInt(Config.EXTRA_ALL_FOLDER_SIZE, folderWindow.getFolder(0).getImageNum());
            }
    ...
    private void initPageModel(List<LocalMediaFolder> folders) {
        if (folders != null) {
                    ...
                    (OnQueryDataResultListener<LocalMedia>) (data, currentPage, isHasMore) -> {
                        if (!isFinishing()) {
                            ...
                            if (mAdapter != null) {
                                ...
                                boolean isEmpty = mAdapter.isDataEmpty();
                                if (isEmpty) {
                                    showDataNull(getString(R.string.picture_empty), R.mipmap.picture_icon_no_data);
                                } else {
                                    hideDataNull();
                                }
            ...
            showDataNull(getString(R.string.picture_data_exception), R.mipmap.picture_icon_data_error);
            dismissDialog();
        }
    }
    ...
    public void startCamera() {
        if (!DoubleUtils.isFastDoubleClick()) {
            if (PictureSelectionConfig.onCustomCameraInterfaceListener != null) {
                if (config.chooseMode == Config.TYPE_ALL) {
    ...
    private void startCustomCamera() {
        if (PermissionChecker.checkSelfPermission(this, Manifest.permission.RECORD_AUDIO)) {
            ...
            overridePendingTransition(windowAnimationStyle.activityEnterAnimation, R.anim.mx_anim_fade_in);
        } else {
            PermissionChecker
                    .requestPermissions(this,
                            new String[]{Manifest.permission.RECORD_AUDIO}, Config.APPLY_RECORD_AUDIO_PERMISSIONS_CODE);
        }
    }
    ...
    private void onPreview() {
        ...
        bundle.putParcelableArrayList(Config.EXTRA_PREVIEW_SELECT_LIST, (ArrayList<? extends Parcelable>) medias);
        bundle.putParcelableArrayList(Config.EXTRA_SELECT_LIST, (ArrayList<? extends Parcelable>) selectedData);
        bundle.putBoolean(Config.EXTRA_BOTTOM_PREVIEW, true);
        bundle.putBoolean(Config.EXTRA_CHANGE_ORIGINAL, config.isCheckOriginalImage);
        bundle.putBoolean(Config.EXTRA_SHOW_CAMERA, mAdapter.isShowCamera());
        bundle.putString(Config.EXTRA_IS_CURRENT_DIRECTORY, mTvPictureTitle.getText().toString());
        JumpUtils.startPicturePreviewActivity(getContext(), config.isWeChatStyle, bundle,
                config.selectionMode == Config.SINGLE ? Config.REQUEST_CROP : Config.REQUEST_MULTI_CROP);
...
//                R.anim.picture_anim_fade_in);
    ...
    public void onTakePhoto() {
        ...
        if (PermissionChecker.checkSelfPermission(this, Manifest.permission.CAMERA)) {
            if (PermissionChecker
                ...
                PermissionChecker.requestPermissions(this, new String[]{
                        Manifest.permission.READ_EXTERNAL_STORAGE,
                        Manifest.permission.WRITE_EXTERNAL_STORAGE}, Config.APPLY_CAMERA_STORAGE_PERMISSIONS_CODE);
            }
        } else {
            PermissionChecker
                    .requestPermissions(this,
                            new String[]{Manifest.permission.CAMERA}, Config.APPLY_CAMERA_PERMISSIONS_CODE);
        }
    }
    ...
    public void onPictureClick(LocalMedia media, int position) {
        if (config.selectionMode == Config.SINGLE && config.isSingleDirectReturn) {
    ...
    public void startPreview(List<LocalMedia> previewData, int position) {
        ...
        if (PictureMimeType.isHasVideo(mimeType)) {
            // video
            if (config.selectionMode == Config.SINGLE && !config.enPreviewVideo) {
                result.add(media);
                onResult(result);
            } else {
                if (PictureSelectionConfig.customVideoPlayCallback != null) {
                    ...
                    bundle.putParcelable(Config.EXTRA_MEDIA_KEY, media);
                    JumpUtils.startPictureVideoPlayActivity(getContext(), bundle, Config.PREVIEW_VIDEO_CODE);
                }
            }
            ...
            bundle.putParcelableArrayList(Config.EXTRA_SELECT_LIST, (ArrayList<? extends Parcelable>) selectedData);
            bundle.putInt(Config.EXTRA_POSITION, position);
            bundle.putBoolean(Config.EXTRA_CHANGE_ORIGINAL, config.isCheckOriginalImage);
            bundle.putBoolean(Config.EXTRA_SHOW_CAMERA, mAdapter.isShowCamera());
            bundle.putLong(Config.EXTRA_BUCKET_ID, ValueOf.toLong(mTvPictureTitle.getTag(R.id.view_tag)));
            bundle.putInt(Config.EXTRA_PAGE, mPage);
            bundle.putParcelable(Config.EXTRA_CONFIG, config);
            bundle.putInt(Config.EXTRA_DATA_COUNT, ValueOf.toInt(mTvPictureTitle.getTag(R.id.view_count_tag)));
            bundle.putString(Config.EXTRA_IS_CURRENT_DIRECTORY, mTvPictureTitle.getText().toString());
            JumpUtils.startPicturePreviewActivity(getContext(), config.isWeChatStyle, bundle,
                    config.selectionMode == Config.SINGLE ? Config.REQUEST_CROP : Config.REQUEST_MULTI_CROP);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                case Config.PREVIEW_VIDEO_CODE:
                    if (data != null) {
                        List<LocalMedia> list = data.getParcelableArrayListExtra(Config.EXTRA_SELECT_LIST);
                        if (list != null && list.size() > 0) {
                            onResult(list);
                        }
                    }
                    break;
                case Config.REQUEST_CROP:
                    singleCropHandleResult(data);
                    break;
                case Config.REQUEST_MULTI_CROP:
    ...
    private void previewCallback(Intent data) {
        ...
        if (config.isOriginalControl) {
            config.isCheckOriginalImage = data.getBooleanExtra(Config.EXTRA_CHANGE_ORIGINAL, config.isCheckOriginalImage);
            mCbOriginal.setChecked(config.isCheckOriginalImage);
        }
        List<LocalMedia> list = data.getParcelableArrayListExtra(Config.EXTRA_SELECT_LIST);
        if (mAdapter != null && list != null) {
            boolean isCompleteOrSelected = data.getBooleanExtra(Config.EXTRA_COMPLETE_SELECTED, false);
    ...
    private void dispatchHandleCamera(Intent intent) {
        ...
        PictureSelectionConfig selectionConfig = intent != null ? intent.getParcelableExtra(Config.EXTRA_CONFIG) : null;
            ...
            public LocalMedia doInBackground() {
                ...
                if (!isAudio) {
                    if (PictureMimeType.isContent(config.cameraPath)) {
                        ...
                        String mediaPath = intent != null ? intent.getStringExtra(Config.EXTRA_MEDIA_PATH) : null;
    ...
    protected void multiCropHandleResult(Intent data) {
        ...
        List<CutInfo> mCuts = data.getParcelableArrayListExtra(".OutputUriList");
    ...
    public void onItemClick(View view, int position) {
        switch (position) {
            case PhotoItemSelectedDialog.IMAGE_CAMERA:
                if (PictureSelectionConfig.onCustomCameraInterfaceListener != null) {
                    PictureSelectionConfig.onCustomCameraInterfaceListener.onCameraClick(getContext(), config, Config.TYPE_IMAGE);
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions,
        ...
        switch (requestCode) {
            case Config.APPLY_STORAGE_PERMISSIONS_CODE:
                // Store Permissions
                ...
                break;
            case Config.APPLY_CAMERA_PERMISSIONS_CODE:
                // Camera Permissions
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    onTakePhoto();
                } else {
                ...
                break;
            case Config.APPLY_CAMERA_STORAGE_PERMISSIONS_CODE:
                ...
                break;
            case Config.APPLY_RECORD_AUDIO_PERMISSIONS_CODE:
