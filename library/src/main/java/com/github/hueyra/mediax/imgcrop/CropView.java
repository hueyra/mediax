package com.github.hueyra.mediax.imgcrop;
...
public class CropView extends View {
    ...
    //裁剪框类别，（圆形、矩形），默认为圆形
    private CropType cropType = CropType.CIRCLE;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (cropType == CropType.CIRCLE) {
            //中间的透明的圆
            canvas.drawCircle(this.getWidth() / 2, this.getHeight() / 2, cropRadiusWidth, paint);
            //白色的圆边框
            canvas.drawCircle(this.getWidth() / 2, this.getHeight() / 2, cropRadiusWidth, borderPaint);
        } else if (cropType == CropType.RECTANGLE) { //绘制矩形裁剪框
            //绘制中间白色的矩形蒙层
            canvas.drawRect(mHorizontalPadding, this.getHeight() / 2 - cropWidth / 2,
                    this.getWidth() - mHorizontalPadding, this.getHeight() / 2 + cropWidth / 2, paint);
            //绘制白色的矩形边框
            canvas.drawRect(mHorizontalPadding, this.getHeight() / 2 - cropWidth / 2,
                    this.getWidth() - mHorizontalPadding, this.getHeight() / 2 + cropWidth / 2, borderPaint);
        } else if (cropType == CropType.PALACE) {
            //绘制中间的矩形
            canvas.drawRect(mHorizontalPadding, this.getHeight() / 2 - cropWidth / 2,
                    this.getWidth() - mHorizontalPadding, this.getHeight() / 2 + cropWidth / 2, paint);
    ...
    public Rect getClipRect() {
        Rect rect = new Rect();
        //宽度的一半 - 圆的半径
        rect.left = (this.getWidth() / 2 - cropRadiusWidth);
        //宽度的一半 + 圆的半径
        rect.right = (this.getWidth() / 2 + cropRadiusWidth);
        //高度的一半 - 圆的半径
        rect.top = (this.getHeight() / 2 - cropRadiusWidth);
        //高度的一半 + 圆的半径
        rect.bottom = (this.getHeight() / 2 + cropRadiusWidth);
        return rect;
    }
    ...
    public void setClipBorderWidth(int clipBorderWidth) {
        this.cropBorderWidth = clipBorderWidth;
    ...
    public void setmHorizontalPadding(float mHorizontalPadding) {
        ...
        this.cropRadiusWidth = (int) (getScreenWidth(getContext()) - 2 * mHorizontalPadding) / 2;
        this.cropWidth = cropRadiusWidth * 2;
    }
    ...
    public void setClipType(int type) {
        switch (type) {
            case TYPE_ROUND:
                this.setClipType(CropType.CIRCLE);
                break;
            case TYPE_RECT:
                this.setClipType(CropType.RECTANGLE);
                break;
            case TYPE_PALACE:
            default:
                this.setClipType(CropType.PALACE);
                break;
        }
    }
     ...
     *
     */
    public void setClipType(CropType cropType) {
    ...
    private void initData() {
        ...
        mPalaceBorderPaint.setStrokeWidth(3f);
        ...
        mCornerPaint.setStrokeWidth(8f);
        ...
        mCornerLength = dip2px(context, 15);
    }
